...
def invert_affine(metas: Union[float, list, tuple], preds):
    for i in range(len(preds)):
        if len(preds[i]['rois']) == 0:
            ...
            if isinstance(metas, (int, float)):
                preds[i]['rois'][:, [0, 2]] = preds[i]['rois'][:, [0, 2]] / metas
                preds[i]['rois'][:, [1, 3]] = preds[i]['rois'][:, [1, 3]] / metas
            elif isinstance(metas[0], (int, float)):
                preds[i]['rois'][:, [0, 2]] = preds[i]['rois'][:, [0, 2]] / metas[i]
                preds[i]['rois'][:, [1, 3]] = preds[i]['rois'][:, [1, 3]] / metas[i]
            else:
...
def display(preds, imgs, obj_list, imshow=True, imwrite=False, out_path='test/'):
    for i in range(len(imgs)):
        ...
        if imwrite:
            ...
            cv2.imwrite(f'{out_path}/{uuid.uuid4().hex}.jpg', imgs[i])
